...
class TestDrivenAdvent < Minitest::Test
  def test_digits_only_calibration_values
    assert_equal 12, Calibration.new("1abc2", DigitsOnly).value
    assert_equal 38, Calibration.new("pqr3stu8vwx", DigitsOnly).value
    assert_equal 15, Calibration.new("a1b2c3d4e5f", DigitsOnly).value
    assert_equal 77, Calibration.new("treb7uchet", DigitsOnly).value
  end

  def test_words_to_digits_calibration_values
    assert_equal 29, Calibration.new("two1nine", WordsToDigits).value
    assert_equal 83, Calibration.new("eightwothree", WordsToDigits).value
    assert_equal 13, Calibration.new("abcone2threexyz", WordsToDigits).value
    assert_equal 24, Calibration.new("xtwone3four", WordsToDigits).value
    assert_equal 42, Calibration.new("4nineeightseven2", WordsToDigits).value
    assert_equal 14, Calibration.new("zoneight234", WordsToDigits).value
    assert_equal 76, Calibration.new("7pqrstsixteen", WordsToDigits).value
  end

  def test_digits_only_calibration_document
    input = <<~INPUT
      1abc2
      pqr3stu8vwx
      a1b2c3d4e5f
    ...
    assert_equal 142, CalibrationDocument.new(input, DigitsOnly).sum
  end

  def test_words_to_digits_calibration_document
    input = <<~INPUT
      two1nine
      eightwothree
      abcone2threexyz
    ...
    assert_equal 281, CalibrationDocument.new(input, WordsToDigits).sum
  end

  def test_found_answers
    input = DATA.read.strip
    assert_equal 54_159, CalibrationDocument.new(input, DigitsOnly).sum
    assert_equal 53_866, CalibrationDocument.new(input, WordsToDigits).sum
  end
end
...
class CalibrationDocument
  ...
  def sum
    input.lines.map { |l| Calibration.new(l, strategy).value }.sum
  end
end
...
class Calibration
  ...
  def value
    transformed_input = strategy.new(input).output
    digits = transformed_input.chars.select { |c| c.to_i.nonzero? }
...
class WordsToDigits
  ...
  def output(string = input)
    return "" if string.size.zero?

    head = string.chars.first
    tail = string.chars.drop(1).join

    SUBSTITUTIONS.find do |word, digit|
      head = digit if string.start_with?(word)
    end
